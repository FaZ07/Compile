...
function extractFacts(intent: Intent, r: Record<string, NodeResult>) {
  ...
  const flightPrice = flights[0]?.price_inr ?? 0;
  const stayTotal = stays[0] ? stays[0].all_in_nightly_inr * nights : 0;
  const computed_total = flightPrice * intent.party_size + stayTotal;
...
function template(intent: Intent, f: ReturnType<typeof extractFacts>): Synthesis {
  ...
  return {
    ...
    summary: `${intent.destination} for ${intent.party_size} over ${nights} night${nights > 1 ? "s" : ""}: cheapest flight is ${f.cheapest_flight?.airline ?? "—"} at ₹${flight}/person. Stay in ${f.cheapest_stay?.area ?? "town"} at ₹${stay}/night all-in.${budgetNote} ${f.weather ? f.weather.summary + "." : ""}`,
